...
from postreise.analyze.binding import \
    pmin_constraints, pmax_constraints, ramp_constraints, \
    _check_scenario, _check_epsilon


def _check_dataframes_match(returned, expected):
    assert isinstance(returned, pd.DataFrame)
    assert returned.to_dict() == expected.to_dict()
...
class TestRampConstraints(unittest.TestCase):
    ...
    def test_ramp_constraints_default(self):
        ...
        _check_dataframes_match(binding_ramps, expected)
...
class TestPminConstraints(unittest.TestCase):
    ...
    def test_pmin_constraints_default(self):
        ...
        expected = self.default_expected.copy()
        _check_dataframes_match(binding_pmins, expected)

    def test_pmin_constraints_default_sepc_epsilon1(self):
        binding_pmins = pmin_constraints(self.mock_scenario, epsilon=1e-3)
        expected = self.default_expected.copy()
        _check_dataframes_match(binding_pmins, expected)

    def test_pmin_constraints_default_sepc_epsilon2(self):
        binding_pmins = pmin_constraints(self.mock_scenario, epsilon=1e-6)
        expected = self.default_expected.copy()
        expected.loc['t1', 'B'] = False
        _check_dataframes_match(binding_pmins, expected)

    def test_pmin_constraints_default_sepc_epsilon3(self):
