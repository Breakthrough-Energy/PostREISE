...
def generate_carbon_stats(pg, grid, name=None):
    ...
    carbon = pd.DataFrame().reindex_like(pg)
    
    coal_indices = (grid.genfuel == 'coal')
    ng_indices = (grid.genfuel == 'ng')
    # Can't do dfo yet because input data says zero fuel cost

    heat[:, coal_indices] = (
        costs[:, coal_indices] / grid.gen['GenFuelCost'].values[coal_indices])
    heat[:, ng_indices] = (
        costs[:, ng_indices] / grid.gen['GenFuelCost'].values[ng_indices])
    ...
    carbon.iloc[:, coal_indices] = heat[:, coal_indices] * 26.05 * 44/12 / 1000
    carbon.iloc[:, ng_indices] = heat[:, coal_indices] * 14.46 * 44/12 / 1000
...
def calc_costs(pg, gencost):
    ...
    # get polynomials in columns, discarding non-coeff data
    coefs = gencost.values.T[:-4:-1,:]
