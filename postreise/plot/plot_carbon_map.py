...
def plot_states(state_list, col_list, labels_list, font_size,
    ...
    for i in state_list:
        ...
        p.patches(a1, b1,
                  fill_alpha=0.8, fill_color=[col_list[n]],
                  line_color="black", line_width=2)
        citation = Label(x=min(a1[0])+100000,
...
def group_zone(bus_map):
    ...
    bus_map1['zone_id'] = bus_map1['zone_id'].replace([
        203, 204, 205, 206], 207)
    ...
    bus_map1['y'] = (bus_map1['max_y'] -
                     bus_map1['min_y']) / 4+bus_map1['min_y']

    return bus_map1


def map_carbon_emission_bar(bus_info_and_emission, scenario_name,
    ...
    width = 150000.0
    alpha = 0.7
    p.vbar(x='x2', bottom='y', top='coaly', width=width,
           color=color_coal,
           source=bus_source, alpha=alpha)
    p.vbar(x='x', bottom='y', top='ngy', width=width,
           color=color_ng,
           source=bus_source, alpha=alpha)
    ...
    cit_y.append(np.add([i+va * 2 for i in bus_map.y.values.tolist()],
                        [i * size_factor for i in
                         bus_map.ng.values.tolist()]))
    cit_y.append(np.add([i+va * 2 for i in bus_map.y.values.tolist()],
                        [i * size_factor for i in
                         bus_map.coal.values.tolist()]))
    cit_text.append([round(num, 1) for num in
                     [i / m for i in bus_map.ng.values.tolist()]])
    cit_text.append([round(num, 1) for num in
                     [i / m for i in bus_map.coal.values.tolist()]])

    for j in range(0, len(cit_x)):
        for i in range(0, len(cit_x[j])):
            citation = Label(x=cit_x[j][i],
                             y=cit_y[j][i],
                             x_units='data', y_units='data',
                             text_font_size='20pt',
    ...
    # custom legend
    p_legend.square(1, 1, fill_color='white',
                    color='white', size=600)
    p_legend.square(1, 1, fill_color='white',
                    color='black', size=400)
    p_legend.square(0.4, 0.8,
                    fill_color='black', color='black',
                    size=40, alpha=0.7)
    p_legend.square(0.4, 0.2,
                    fill_color='purple', color='purple',
                    size=40, alpha=0.7)
    source = ColumnDataSource(
        data=dict(x=[.7, .7, 0.05],
                  y=[.6, 0.1, 1.5],
                  text=['Coal', 'Natural Gas',
                        'Emissions (Million Tons)']))
    labels = LabelSet(x='x', y='y', text='text',
                      source=source, level='glyph',
...
def map_carbon_emission(bus_info_and_emission, scenario_name,
    ...
    # Set up figure
    tools: str = "pan,wheel_zoom,reset,hover,save"
    p = figure(title=scenario_name, tools=tools,
               x_axis_location=None,
               y_axis_location=None,
               plot_width=800, plot_height=800, output_backend="webgl")
    p_legend = figure(x_axis_location=None,
                      y_axis_location=None,
                      toolbar_location=None,
    ...
    p.patches(a, b, fill_alpha=0.0,
              line_color="black", line_width=2)
    # emissions circles
    p.circle('x', 'y', fill_color=color_coal,
             color=color_coal, alpha=0.25,
             size='coal', source=bus_source)
    p.circle('x', 'y', fill_color=color_ng,
             color=color_ng, alpha=0.25,
    ...
    p_legend.circle(1, y=np.repeat([1, 3], 3),
                    ...
                    alpha=0.25,
                    size=[(10000000 / 1000 * size_factor) ** 0.5,
                          (5000000 / 1000 * size_factor) ** 0.5,
                          (1000000 / 1000 * size_factor) ** 0.5] * 2)
    source = ColumnDataSource(
...
def map_carbon_emission_comparison(bus_info_and_emission_1,
    ...
    bus_map = bus_info_and_emission_1.merge(
        bus_info_and_emission_2, right_index=True,
...
def combine_bus_info_and_emission(bus_info, carbon_by_bus):
    ...
    bus_of_interest = bus_info.loc[pd.DataFrame.from_dict(
        carbon_by_bus).index]
    bus_info_and_emission = bus_of_interest.merge(
        pd.DataFrame.from_dict(carbon_by_bus),
        right_index=True,
        left_index=True)
