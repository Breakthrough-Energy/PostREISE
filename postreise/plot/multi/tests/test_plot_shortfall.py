...
from postreise.plot.multi.plot_shortfall import (
    _construct_shortfall_ax_data, _construct_shortfall_data_for_western,
...
def test_construct_shortfall_ax_data():
    ax_data, shortfall_pct_list = _construct_shortfall_ax_data(
        'Washington', create_mock_graph_data(), is_match_CA=False, baseline=4, target=7, demand=20)
    expected_data = {
...
def test_construct_shortfall_ax_data_when_shortfall_is_zero():
    ax_data, shortfall_pct_list = _construct_shortfall_ax_data(
        'Washington', create_mock_graph_data(), is_match_CA=False, baseline=4, target=5, demand=20)
    expected_data = {
        '2016 Simulated Base Case': {
            '2016 Renewables': 4,
            'Simulated increase in renewables': 2.0,
...
def test_construct_shortfall_ax_data_when_target_is_zero():
    ax_data, shortfall_pct_list = _construct_shortfall_ax_data(
        'Washington', create_mock_graph_data(), is_match_CA=False, baseline=4, target=0, demand=20)
    expected_data = {
        '2016 Simulated Base Case': {
            '2016 Renewables': 4,
            'Simulated increase in renewables': 2.0,
...
def test_construct_shortfall_data_for_western_with_independent_scenario():
    ...
    ax_data, shortfall_pct_list = _construct_shortfall_data_for_western(create_mock_graph_data(
        more_gen=True), is_match_CA=False, has_collaborative_scenarios=None, baseline=13, targets=mock_targets, demand=50)
    expected_data = {
        '2016 Simulated Base Case': {
            '2016 Renewables': 13,
...
def test_construct_shortfall_data_for_western_with_collaborative_scenarios():
    ...
    collab = ['87', '2016_nrel']
    ax_data, shortfall_pct_list = _construct_shortfall_data_for_western(create_mock_graph_data(
        more_gen=True), is_match_CA=False, has_collaborative_scenarios=collab, baseline=13, targets=mock_targets, demand=50)
    expected_data = {
        '2016 Simulated Base Case': {
            '2016 Renewables': 13,
            # western target - shortfall - baseline
