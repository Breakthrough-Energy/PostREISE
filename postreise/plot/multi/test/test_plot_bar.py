...
def test_construct_bar_ax_data_fills_in_missing_resources_with_0():
    ...
    assert ax_data_list[1]['values'] == {'2016 Simulated Base Case': [0,1,5,4], '2016 NREL Data': [2,1,3,4]}
...
def test_construct_bar_ax_data_with_user_set_resource_types():
    ax_data_list = _construct_bar_ax_data('Washington', create_mock_graph_data(), ['wind', 'solar'])
    expected_output = [{
        'title': 'Generation (TWh)',
        'labels': ['Wind', 'Solar'],
...
def test_get_bar_resource_types_handles_resource_types_not_found_in_known_resource_types():
    ...
    mock_graph_data['87']['gen']['data']['Washington'] = {'cereal': 1, 'coal': 2, 'milk': 1, 'bananas': 3}
    resource_types = _get_bar_resource_types('Washington', mock_graph_data)
    assert resource_types == ['wind', 'solar', 'coal', 'hydro', 'bananas', 'cereal', 'milk']
