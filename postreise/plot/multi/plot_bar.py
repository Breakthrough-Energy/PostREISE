...
def plot_hbar(zone_name, scenario_ids=None, custom_data=None, resource_types=None):
    zone_list, graph_data = handle_plot_inputs(zone_name, scenario_ids, custom_data)
    for zone in zone_list:
        ax_data_list = _construct_bar_ax_data(zone, graph_data, resource_types)
...
def _construct_bar_ax_data(zone, scenarios, user_set_resource_types):
    resource_types = user_set_resource_types if user_set_resource_types != None else _get_bar_resource_types(zone, scenarios)
    ...
    for side in ['gen', 'cap']:
        ...
        for scenario in scenarios.values():
            ...
            ax_data[label] = [data_for_zone[resource] if resource in data_for_zone.keys() else 0 for resource in resource_types]
...
def _construct_bar_visuals(zone, ax_data_list):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(1.5*num_scenarios*num_resource_types, 6))
    ...
    for ax_data, ax in zip(ax_data_list, axes):
        ...
        for p in ax.patches:
            b = p.get_bbox()
            ax.annotate(_get_bar_display_val(b.y1), ((b.x1 + b.x0)/2, b.y1 + 0.02*ax.get_ylim()[1]), fontsize=10, rotation='horizontal', horizontalalignment='center')
...
def _construct_hbar_visuals(zone, ax_data_list):
    ...
    for ax_data, ax in zip(ax_data_list, axes):
        ...
        for p in ax.patches:
            b = p.get_bbox()
            ax.annotate(_get_bar_display_val(b.x1), (b.x1, b.y1-.02), fontsize=14, verticalalignment='top')
