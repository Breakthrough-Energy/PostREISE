...
def makemap_carbon(bus_map, scenario_name, color1='black',
                   ...
                   label2="NG: tons"):
    """Make map of carbon emissions, color code by fuel type, size/area indicates emissions
    :param pandas.DataFrame bus_map: emissions and coordinates of buses
    ...
    :return bokeh.figure: map of carbon emissions per bus color code by type
    """
...
def makemap_carbon_compare(bus_map1, bus_map2, scenario_name1, scenario_name2):
    """Make map of carbon emissions, color code by fuel type, size/area indicates emissions
    :param pandas.DataFrame bus_map1: emissions and coordinates of buses, scenario1
    :param pandas.DataFrame bus_map2: emissions and coordinates of buses, scenario2
    :param str scenario_name2: name of scenario 1 for labeling
    :param str scenario_name2: name of scenario 2 for labeling
    :return bokeh.figure: figures for each scenario carbon emissions plus a coomparison map 
        indicating increase or decrease
    """
    makemap_carbon(bus_map1, scenario_name1)
    makemap_carbon(bus_map2, scenario_name2)

    bus_map_merge = bus_map1.merge(
        bus_map2, right_index=True, left_index=True, how='outer')
    ...
    bus_map_merge['coal_dif'] = bus_map_merge['coal_x'] - \
        bus_map_merge['coal_y']
    ...
    bus_map_merge["ng"] = 0
    bus_map_merge.loc[bus_map_merge.coal_dif >
                      0, ['coal']] = bus_map_merge['coal_dif']
    bus_map_merge
    bus_map_merge.loc[bus_map_merge.ng_dif >
                      0, ['coal']] = bus_map_merge['ng_dif']
    bus_map_merge

    bus_map_merge.loc[bus_map_merge.coal_dif < 0,
                      ['ng']] = abs(bus_map_merge['coal_dif'])
    bus_map_merge
    bus_map_merge.loc[bus_map_merge.ng_dif < 0,
                      ['ng']] = abs(bus_map_merge['ng_dif'])
    bus_map_merge

    makemap_carbon(bus_map_merge, "compare", "green",
                   "red", "Less: tons", "More: tons")
