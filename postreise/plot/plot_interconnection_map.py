...
def map_interconnections(grid, us_states_dat=us_states.data):
    ...
    branch_mdc['from_lon'] = branch_bus.loc[
        branch_mdc.from_bus_id, 'lon'].values
    branch_mdc['from_lat'] = branch_bus.loc[
        branch_mdc.from_bus_id, 'lat'].values
    ...
    p = figure(title='Interconnections', tools=tools, x_axis_location=None,
               y_axis_location=None,
               plot_width=800, plot_height=800, output_backend="webgl")

    # for legend
    p.multi_line([-1.084288e+07, -1.084288e+07], [4.639031e+06, 4.639031e+06],
                 color='blue', line_width=5, legend="Western")
    p.multi_line([-1.084288e+07, -1.084288e+07], [4.639031e+06, 4.639031e+06],
                 color='red', line_width=5, legend="Eastern")
    p.multi_line([-1.084288e+07, -1.084288e+07], [4.639031e+06, 4.639031e+06],
                 color='purple', line_width=5, legend="ERCOT")
    p.multi_line([-1.084288e+07, -1.084288e+07], [4.639031e+06, 4.639031e+06],
                 color='green', line_width=5, legend="HVDC")
    p.square(x=[-1.084288e+07], y=[4.639031e+06], size=170, fill_color="white",
             color="white")
    ...
    #branches
    p.multi_line('xs', 'ys', color='blue', line_width='capacity',
                 source=multi_line_source)
    p.multi_line('xs','ys', color='red', line_width='capacity',
                 source=multi_line_source2)
    p.multi_line('xs', 'ys', color='purple', line_width='capacity',
                 source=multi_line_source3)
    p.multi_line('xs', 'ys', color='green', line_width='capacity',
                 source=multi_line_source4)
