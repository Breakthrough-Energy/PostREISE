...
def test_construct_bus_data():
    ...
    expected_bus_segments = [{2: {'lmp': 5.5, 'x': 2.0, 'y': 2.0},
                              3: {'lmp': 8.3, 'x': 3.0, 'y': 3.0}},
...
def test_get_lmp_split_points_with_min_lmp_below_neg_one():
    lmp_vals = [-5, -2, 0, 2, 2, 6, 6, 10, 10,
                12, 12, 14, 14, 16, 16, 18, 18, 20, 20]
    random.shuffle(lmp_vals)
    mock_bus_map = pd.DataFrame(
        {bus_id: {'lmp': lmp} for bus_id, lmp in
         zip(range(len(lmp_vals)), lmp_vals)}).T

    lmp_split_points = _get_lmp_split_points(mock_bus_map)
    assert lmp_split_points == [-5, -1, 1, 6.0, 10.0, 12.0, 14.0, 16.0, 18.0,
                                20]
...
def test_get_lmp_split_points_with_min_lmp_more_than_one():
    ...
    lmp_split_points = _get_lmp_split_points(mock_bus_map)
    assert lmp_split_points == [100, 200.0, 300.0, 400.0, 500.0, 600.0, 700.0,
                                800.0, 900.0, 1000]
...
def test_construct_branch_data():
    ...
    mock_branch_map = pd.DataFrame({
        1: {'from_x': 1, 'to_x': 10, 'from_y': 1, 'to_y': 10,
            'branch_device_type': 'Line'},
        2: {'from_x': 2, 'to_x': 20, 'from_y': 2, 'to_y': 20,
            'branch_device_type': 'Line'},
        3: {'from_x': 3, 'to_x': 30, 'from_y': 3, 'to_y': 30,
            'branch_device_type': 'Line'},
        4: {'from_x': 4, 'to_x': 40, 'from_y': 4, 'to_y': 40,
    ...
    branches_selected = _construct_branch_data(
        mock_branch_map, mock_cong, hour)
    expected_branches_selected = {1: {'from_x': 1,
                                      'to_x': 10,
                                      'from_y': 1,
                                      'to_y': 10,
                                      'branch_device_type': 'Line',
