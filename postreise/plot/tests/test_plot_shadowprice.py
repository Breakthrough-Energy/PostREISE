...
from postreise.plot.plot_shadowprice import (_construct_branch_data,
                                             _construct_bus_data,
...
def test_construct_bus_data():
    ...
    lmp_split_points, bus_segments = _construct_bus_data(mock_bus_map, mock_lmp, mock_split_points, hour)
...
def test_get_lmp_split_points_with_min_lmp_below_neg_one():
    ...
    mock_bus_map = pd.DataFrame(
        {bus_id: {'lmp': lmp} for bus_id, lmp in zip(range(len(lmp_vals)), lmp_vals)}).T
...
def test_get_lmp_split_points_with_min_lmp_more_than_one():
    ...
    lmp_split_points = _get_lmp_split_points(mock_bus_map)
    assert lmp_split_points == [100, 200.0, 300.0, 400.0, 500.0, 600.0, 700.0, 800.0, 900.0, 1000]
...
def test_construct_branch_data():
    ...
    mock_branch_map = pd.DataFrame({
        1: {'from_x': 1, 'to_x': 10, 'from_y': 1, 'to_y': 10, 'branch_device_type': 'Line'},
        2: {'from_x': 2, 'to_x': 20, 'from_y': 2, 'to_y': 20, 'branch_device_type': 'Line'},
        3: {'from_x': 3, 'to_x': 30, 'from_y': 3, 'to_y': 30, 'branch_device_type': 'Line'},
        4: {'from_x': 4, 'to_x': 40, 'from_y': 4, 'to_y': 40, 'branch_device_type': 'Stick'}
    }).T
    ...
    expected_branches_selected = {
        1: {'from_x': 1, 'to_x': 10, 'from_y': 1, 'to_y': 10, 'branch_device_type': 'Line', 'medianval': 2.0},
        3: {'from_x': 3, 'to_x': 30, 'from_y': 3, 'to_y': 30, 'branch_device_type': 'Line', 'medianval': 8.0}
    }
