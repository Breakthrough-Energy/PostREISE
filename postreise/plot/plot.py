...
def plot_solar(PG, load_zone, from_index='2016', to_index='2016', freq='W'):
    """Plots the time series of the solar power generated for load zone, California or total
    western interconnect. Also show on the same plot the time series of the aggregated 25%,
    50% and 75% most powerful plants (in terms of capacity).
    ...
    daylight_saving = [datetime.strptime('2016/3/13/2', '%Y/%m/%d/%H'),datetime.strptime('2016/11/6/2', '%Y/%m/%d/%H')]
    ...
    for enum, i in enumerate(plantID):
        ...
        if offset == time_offset(2016, 6, 1, 0, 0, 0, WI.genbus.loc[i].lon, WI.genbus.loc[i].lat):
            ...
            lt_tmp.insert(lt_tmp.index(daylight_saving[1]), datetime.strptime('2016/11/6/1', '%Y/%m/%d/%H'))
            PG_lt_tmp['lt'] = lt_tmp
            PG_lt_tmp.set_index('lt', inplace=True, drop=True)
    ...
    if n_plants < 20:
        ax = total.plot(fontsize=18, alpha=0.7, figsize=(18, 12), color=WI.type2color['solar'], lw=5)
        ax.set_facecolor('white')
        ax.grid(color='black', axis='y')
        ...
        ax = total.plot(fontsize=18, alpha=0.7, figsize=(18, 12), color=[WI.type2color['solar'],'r','g','b'], lw=2)
        ax.set_facecolor('white')
        ax.grid(color='black', axis='y')
        ax.set_xlabel('Local Time', fontsize=20)
        handles, labels = ax.get_legend_handles_labels()
