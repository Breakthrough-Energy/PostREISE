...
traffic_palette = ['darkgreen', 'green',
                   'limegreen', 'lawngreen',
                   'yellowgreen', 'yellow',
                   'gold', 'goldenrod',
                   'orange', 'orangered',
...
def map_risk_bind(risk_or_bind, congestion_stats, branch,
    ...
    branch_congestion = pd.concat(
        [branch.loc[congestion_stats.index],
         congestion_stats], axis=1)
    ...
    mapper1 = linear_cmap(field_name=risk_or_bind,
                              palette=traffic_palette, low=min_val,
                              high=max_val)
    color_bar = ColorBar(color_mapper=mapper1['transform'], width=8,
                             location=(0, 0), title=risk_or_bind)
    multi_line_source = ColumnDataSource({
        ...
        # Set up figure
    p = figure(title=risk_or_bind, tools=tools,
    ...
    p.multi_line('xs', 'ys', color='gray', line_width='cap',
                     source=multi_line_source_all, alpha=0.5)
    p.multi_line('xs', 'ys', color=mapper1, line_width='cap',
...
def map_utilization(utilization_df, branch, us_states_dat=us_states.data):
    ...
    branch_utilization = pd.concat(
        [branch.loc[branch.rateA != 0],
         utilization_df[branch.loc[branch.rateA != 0].index].median().rename(
    ...
    mapper1 = linear_cmap(field_name='median_utilization',
